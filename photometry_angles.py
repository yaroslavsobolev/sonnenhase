...
image1 = gaussian_filter(image1, gauss_sigma)
...
for n in range(10):
    ...
    reflection_here = np.max(image1)
    angle_here = angles[n]
    print('{0}\t{1}'.format(angle_here, reflection_here))
    reflections.append(reflection_here)
    fig.savefig('figures/{0:03d}'.format(n))
    plt.show()
